...
Arrow.create = function(options){
	...
	function stackManager(){
		...
		function next( err, msg ){
			...
			/**
			* next() will look for a handler at handlerIndex
			...
			* --------------------------------------------------------
			* If a message is parsed it will be added to the messages collection
			* If err === 'skip' the current layer will be aborded
			...
			*/
			if (!layer || ( err && 'object' === typeof err )){
				...
				if (_final){
					return _final.apply(context, args)
				}
				return { err : err, msg : messages }
			}
			if (err && err === 'skip'){ /* error is a command */
